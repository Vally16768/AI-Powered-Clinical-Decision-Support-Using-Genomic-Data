...
ENV: Dict[str, Any] = {
    ...
    "LLM_CANDIDATES": _parse_candidates(os.getenv("LLM_CANDIDATES") or "qwen2.5:7b-instruct, qwen2.5:3b-instruct, phi3:mini"),
    "LLM_MIN_VRAM_GB": float(os.getenv("LLM_MIN_VRAM_GB", "6.0")),
    "LLM_TIMEOUT_SECONDS": int(os.getenv("LLM_TIMEOUT_SECONDS", "25")),
    "LLM_TEMPERATURE": float(os.getenv("LLM_TEMPERATURE", "0.2")),
    "LLM_VRAM_CATALOG": (_parse_vram_catalog(os.getenv("LLM_VRAM_CATALOG")) or {
    ...
    "ENABLE_ONCOKB": _as_bool(os.getenv("ENABLE_ONCOKB", "false")),
    ...
    "OPENCRAVAT_API_BASE": os.getenv("OPENCRAVAT_API_BASE", ""),
...
def _score_variant(v: VariantDetails) -> Tuple[float,str,str,VariantEvidence,List[str]]:
    ...
    rationale = f"Variant prioritized as {label} based on: {', '.join(bits)}" if bits else f"Variant prioritized as {label} with limited evidence."
    evidence = VariantEvidence(cadd_norm=cadd_n, polyphen_score=polyphen_score, sift_score=sift_score, clinvar_score=clinvar_score, rules_fired=rules_fired)
...
class VariantAnnotator:
    def __init__(self, csv_path: str):
        ...
        if "cadd" in self.df.columns:
            try: self.df["cadd"] = pd.to_numeric(self.df["cadd"], errors="coerce")
...
# =================== EXTERNAL CONNECTORS ===================
import httpx
from connectors.vep import annotate_vep           # :contentReference[oaicite:0]{index=0}
from connectors.opencravat import annotate_opencravat  # :contentReference[oaicite:1]{index=1}
from connectors.oncokb import annotate_oncokb     # :contentReference[oaicite:2]{index=2}
from connectors.civic import annotate_civic       # :contentReference[oaicite:3]{index=3}
...
def _ollama_generate(model: str, system_prompt: str, user_prompt: str, timeout_s: int, temperature: float)->str:
    ...
    # small retry for transient failures
    last_exc = None
...
def _condense_api_evidence(ev: List[Dict[str, Any]], max_lines: int=6)->List[str]:
    if not ev: return []
    sources: Dict[str, Dict[str, Any]] = {}
    for e in ev:
        src = e.get("source") or "EXT"
        typ = (e.get("type") or "").lower()
        key = (e.get("key") or "").lower()
        val = e.get("value")
        d = sources.setdefault(src, {"impacts": set(), "conseq": set(), "tx": set(), "kv": []})
        if typ == "impact" or key == "impact":
            if val is not None: d["impacts"].add(str(val))
        elif typ == "consequence" or key == "consequence":
            if val is not None: d["conseq"].add(str(val))
        elif typ == "transcript" or key == "transcript":
            if val is not None: d["tx"].add(str(val))
        else:
            if val is not None: d["kv"].append(f"{e.get('key') or e.get('type')}={val}")
    lines=[]
    for src, agg in sources.items():
        parts=[]
        if agg["impacts"]: parts.append("Impact=" + ", ".join(list(agg["impacts"])[:2]))
        if agg["conseq"]: parts.append("Consequence=" + ", ".join(list(agg["conseq"])[:2]))
        if agg["tx"]: parts.append("Transcripts=" + ", ".join(list(agg["tx"])[:2]))
        if agg["kv"]: parts.append(" + ".join(agg["kv"][:2]))
        if parts: lines.append(f"[{src}] " + " | ".join(parts))
...
def build_llm_prompt(ehr: Optional[Dict[str, Any]], pv: List[PrioritizedVariant])->str:
    ...
    for item in pv[:10]:
        v = item.variant
        lines.append(
            f"- {v.gene or 'NA'} {v.variant_id} | priority={item.priority_label} | "
            f"score={round(item.priority_score,3)} | CADD={v.cadd} | PolyPhen={v.polyphen} | "
            f"SIFT={v.sift} | ClinVar={v.clinvar}"
        )
        for k in (v.extra.knowledge or [])[:3]:
        ...
        ev = v.extra.api_evidence or []
        if ev:
            condensed = _condense_api_evidence(ev, max_lines=4)
            if condensed:
...
def _fallback_summary(patient_id: str, pv: List[PrioritizedVariant], ehr: Optional[Dict[str, Any]])->str:
    ...
    for p in top:
        v = p.variant
        kb = next((k.get("url") for k in (v.extra.knowledge or []) if k.get("url")), None)
        bullets.append(f"- {v.gene or 'NA'} {v.variant_id}: {p.priority_label} (score {round(p.priority_score,3)}). "
                       f"Evidence: CADD={v.cadd}, PolyPhen={v.polyphen}, SIFT={v.sift}, ClinVar={v.clinvar}" + (f" [ref]({kb})" if kb else ""))
    return "\n".join(bullets + ["Summary generated by rule-based fallback due to LLM unavailability."])

# =================== FASTAPI ===================
app = FastAPI(title="Genomic CDSS API", version="1.2.1")
...
class RequestContextMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            request.state.duration_ms = int((time.perf_counter() - start) * 1000)
            raise
        request.state.duration_ms = int((time.perf_counter() - start) * 1000)
...
def validate_variants(rows: List[Dict[str, Optional[str]]], *, require_gene: bool)->Optional[JSONResponse]:
    ...
    for idx,r in enumerate(rows):
        if not r.get("variant_id"):
            details.append({"row":idx+1,"field":"variant_id","issue":"missing"}); continue
    ...
    if details: return error("VALIDATION_ERROR","Invalid variant upload", details, status=400)
    return None
...
def validate_ehr(ehr: Dict[str, Any])->Optional[JSONResponse]:
    ...
    if "age" in ehr:
        try:
            if ehr["age"] is not None: ehr["age"] = int(ehr["age"])
        except Exception:
    ...
    if details: return error("VALIDATION_ERROR","Invalid EHR upload", details, status=400)
    return None
...
def _startup()->None:
    ...
    if ENV["LLM_PROVIDER"].upper() == "OLLAMA":
        if MODEL_SELECTED:
            print(f"[LLM] Selected model: {MODEL_SELECTED} (min_vram={ENV['LLM_MIN_VRAM_GB']} GB)")
        else:
            print("[LLM][WARN] No suitable LLM model fits VRAM; fallback may be used.")
...
async def annotate_variant_all_external(variant: Dict[str, Any], ehr: Dict[str, Any])->Dict[str, Any]:
    ...
    async with httpx.AsyncClient() as session:
        ...
        # optional second pass for CIViC if HGVSp short appears in VEP
        if gene and ENV["ENABLE_CIVIC"] and not civ_ev and not civ_kn:
            hgvsp_short = None
            for e in (vep_ev or []):
                if e.get("type") == "hgvsp_short":
                    hgvsp_short = str(e.get("value")); break
            if hgvsp_short:
                ev2, kn2 = await annotate_civic(session, gene, variant_label=var_id, hgvsp_short=hgvsp_short)
                civ_ev, civ_kn = ev2 or [], kn2 or []
    ...
    try:
        print(f"\n[EXT] Variant {var_id} (gene={gene}, disease={disease})")
        print(f"[EXT] Counts -> VEP:{len(vep_ev)} | CRAVAT:{len(cravat_ev)} | OncoKB_ev:{len(onc_ev)} | OncoKB_kn:{len(onc_kn)} | CIViC_ev:{len(civ_ev)} | CIViC_kn:{len(civ_kn)}")
        preview = {"api_evidence_preview": (api_evidence[:5] if api_evidence else []),
                   "knowledge_preview": (ext_knowledge[:5] if ext_knowledge else [])}
        print("[EXT] Preview:", json.dumps(preview, ensure_ascii=False, indent=2))
    except Exception:
        pass
...
async def upload_variants(
    ...
    """
    Accepts VCF or CSV and stores a normalized list of {variant_id, gene?} for the patient.
    - VCF: uses CHROM, POS, REF, ALT (gene optional)
    - CSV: either 'variant_id,gene' or 'CHROM,POS,REF,ALT[,gene]'
    """
    ...
    try:
        if fname.endswith(".vcf"):
            ...
            # Use csv.DictReader to avoid heavy pandas requirement here
            reader = csv.DictReader(StringIO(text))
            for row in reader:
                r = normalize_csv_row(row)
                if r:
                    rows.append(r)
        else:
            # Try to auto-detect CSV (fall back)
            try:
                reader = csv.DictReader(StringIO(text))
                for row in reader:
                    r = normalize_csv_row(row)
                ...
                return error("UNSUPPORTED_FILE", f"Unsupported file type for {file.filename}. Use .vcf or .csv", status=415)
    except Exception as e:
        return error("PARSE_ERROR", f"Failed to parse {file.filename}: {e}", status=400)

    if not rows:
        return error("NO_VARIANTS", "No variants found in uploaded file.", status=400)

    # Validate (gene is optional here; it can be added later by annotator or CSV)
    bad = validate_variants(rows, require_gene=False)
    if bad:
        payload = bad.body
        payload["request_id"] = getattr(request.state, "request_id", str(uuid.uuid4()))
        payload["duration_ms"] = int((time.perf_counter() - _start) * 1000)
    ...
    # Store in memory for this patient
    UPLOAD_STORE.setdefault(patient_id, {})["variants"] = deduped
...
async def upload_ehr(
    ...
    ehr_json: str | None = Form(None),
    request: Request = None
):
    """
    Accepts:
      - CSV with a single row of columns like age,sex,cancer_type,stage
      - JSON (file) or ehr_json (string form field)
    """
    _start = time.perf_counter()
    ...
    if file is not None:
        ...
        try:
            ...
            return error("PARSE_ERROR", f"Failed to parse EHR file {file.filename}: {e}", status=400)

    elif ehr_json is not None:
        try:
            ehr = json.loads(ehr_json)
        except Exception as e:
            return error("PARSE_ERROR", f"Invalid ehr_json: {e}", status=400)
    else:
        return error("NO_EHR", "Provide either a file or ehr_json.", status=400)
...
async def analyze(req: AnalyzeRequest, request: Request):
    ...
    if not rows: return error("NO_VARIANTS","No variants provided (upload first or include in body).", status=400)
    ...
    prioritized: List[PrioritizedVariant] = []
    for v in annotated:
        v.extra.knowledge = (KnowledgeBase(ENV["GENE_KNOWLEDGE_CSV"]).get(v.gene) or [])
        score, label, rationale, evidence, _ = _score_variant(v)
    ...
    ehr_dict = ehr_in or {}
    externals = await asyncio.gather(*[annotate_variant_all_external(item.variant.dict(), ehr_dict) for item in prioritized])
    ...
    if ENV["AUDIT_ENABLED"]:
        ...
        Path(ENV["AUDIT_DIR"], f"{getattr(request.state,'request_id',str(uuid.uuid4()))}.analyze.json").write_text(json.dumps(audit_artifact, indent=2), encoding="utf-8")

    try:
        print("\n[ANALYZE] External enrichment summary per variant:")
        for p in prioritized:
            v = p.variant
            print(f"  - {v.variant_id} ({v.gene}) | api_evidence={len(v.extra.api_evidence)} | knowledge={len(v.extra.knowledge)} | score={p.priority_score} [{p.priority_label}]")
    except Exception:
        pass

    UPLOAD_STORE.setdefault(patient_id, {})["last_prioritized"] = [p.dict() for p in prioritized]
    ...
    return AnalyzeResponse(
        ...
        request_id=getattr(request.state, "request_id", str(uuid.uuid4())),
        duration_ms=_duration_ms,
        ehr=ehr_dict or None,
    )

# =================== LLM SUMMARY (unchanged behavior, with generate retries) ===================
@app.post("/llm_summary", response_model=LLMResponse)
def llm_summary(req: LLMRequest, request: Request):
    _start = time.perf_counter()
    model = choose_llm_model(
        ENV["LLM_CANDIDATES"], ENV["LLM_MIN_VRAM_GB"], ENV["LLM_VRAM_CATALOG"]
    ) if ENV["LLM_PROVIDER"].upper() == "OLLAMA" else None
    ...
    try:
        print("\n=== LLM PROMPT BEGIN ==="); print(prompt); print("=== LLM PROMPT END ===\n")
    except Exception:
        pass
    ...
    try:
        if not model:
            raise RuntimeError("No LLM model selected (choose_llm_model returned None).")
        text = _ollama_generate(
            model, SYSTEM_PROMPT, prompt, ENV["LLM_TIMEOUT_SECONDS"], ENV["LLM_TEMPERATURE"]
        )
    ...
    if ENV["AUDIT_ENABLED"]:
        ...
        audit_artifact = {
            ...
            "llm": {"model": model if model else None, "used_fallback": used_fallback, "error": err_msg, "prompt_sha256": h},
            "duration_ms": _duration_ms,
        }
...
def health_llm():
    """
    Robust Ollama connectivity & models check.
    Returns ok=True only if Ollama is reachable and returns tags (models list).
    Adds extra diagnostics to help the UI.
    """
    info = {
        ...
        "models": [],
        "error": None,
        "selected_model": choose_llm_model(ENV["LLM_CANDIDATES"], ENV["LLM_MIN_VRAM_GB"], ENV["LLM_VRAM_CATALOG"]),
    ...
    if ENV["LLM_PROVIDER"].upper() != "OLLAMA":
        ...
        info["reachable"] = True
        return info
    base = ENV["OLLAMA_HOST"].rstrip("/")
    try:
        # quick version ping
        rv = requests.get(base + "/api/version", timeout=3)
        if rv.status_code < 500:
            info["reachable"] = True
    except Exception as e:
        info["error"] = f"version_check: {type(e).__name__}: {e}"

    try:
        r = requests.get(base + "/api/tags", timeout=5)
        r.raise_for_status()
        tags = r.json().get("models", [])
        info["models"] = [m.get("name") for m in tags if m.get("name")]
        info["ok"] = True
        info["error"] = None
    except Exception as e:
        info["error"] = f"tags_check: {type(e).__name__}: {e}"
    return info
...
def health_all():
    ...
    return {
        ...
        "service": {"name": "Genomic CDSS API", "version": "1.2.1"},
    }
...
@app.get("/")
def root():
    return {"ok": True, "name": "Genomic CDSS API", "version": "1.2.1", "policy_version": POLICY_VERSION}
